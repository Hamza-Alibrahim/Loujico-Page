...
<nav className="fixed top-0 w-full bg-white text-gray-800 p-4 z-[100]">
    ...
    <ul className="flex flex-col space-y-0 mt-12 text-2xl">
        {NavLinks.map((link) => (
        <li key={link.id} className="w-full border-b border-gray-200 last:border-b-0">
            {link.title === "Portfolio" ? (
            <div>
                <button onClick={togglePortfolio} className="w-full flex justify-between items-center p-4 text-[#1F476F] hover:text-[#B88647] transition duration-300">
                <span>{link.title}</span>
                <FaChevronDown className={`transition-transform duration-300 ${isPortfolioOpen ? 'rotate-180' : 'rotate-0'}`} />
                </button>
                {isPortfolioOpen && (
                <ul className="flex flex-col bg-gray-100 items-center space-y-0 text-lg">
                    {PortfolioDropdownLinks.map((dropdownLink) => (
                    <li key={dropdownLink.id} className="w-full border-b border-gray-300 last:border-b-0">
                        <a 
